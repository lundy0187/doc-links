# doc-links
Reference library of links to openly available literature over the internet.  

## DSP Theory

### Basic DSP Theory
- [Engineering Signal Analysis eBook](https://books.open.tudelft.nl/home/catalog/book/247)  
- [ARM: Digital Signal Processing using Arm Cortex-M based Microcontrollers eBook](https://github.com/arm-university/Digital-Signal-Processing-using-Arm-Cortex-M-based-Microcontrollers)  
- [Signal Processing for Communications eBook](https://www.sp4comm.org/download.html)  

### Advanced DSP Theory
- [Foundations of Signal Processing and Wavelets eBook](https://fourierandwavelets.org/)  
- [Introduction to Statistical Signal Processing eBook](https://ee.stanford.edu/~gray/sp.html)  

## RF/Microwave Engineering Theory

- [TU Eindhoven: Modern Antennas and Microwave Circuits eBook](https://arxiv.org/abs/1911.08484)

### Massive Open Online Courses
- [TU Eindhoven: Microwave engineering and antennas Course](https://www.coursera.org/learn/microwave-antenna)  
- [TU Eindhoven: RF and millimeter-Wave Circuit Design Course](https://www.coursera.org/learn/rf-mmwave-circuit-design)  

## RF Applications

### Radar and EW
- [MIT/LL: Intro to Radar Course](https://www.ll.mit.edu/outreach/radar-introduction-radar-systems-online-course)  
- [DTIC: Electronic Warfare and Radar Systems Engineering eBook](https://apps.dtic.mil/sti/citations/ADA617071)  

### Software Defined Radio 
- [Analog Devices: SDR for Engineers eBook](https://www.analog.com/en/resources/technical-books/software-defined-radio-for-engineers.html)  
- [PySDR eBook](https://pysdr.org/)  

#### Computer Networks
- [Computer Networking Course](https://gaia.cs.umass.edu/kurose_ross/lectures.php)  

#### Open Source Software References
- [AFF3CT: A Fast Forward Error Correction Toolbox](https://aff3ct.github.io/)  
- [USRP Hardware Driver and USRP Manual](https://files.ettus.com/manual/index.html)  

#### Cyclic Redundancy Check References
- [A Painless Guide to CRC Error Detection Algorithms eBook](http://ross.net/crc/download/crc_v3.txt)  
- [Analog Devices: CRC Programming MAX14900E Note](https://www.analog.com/en/resources/app-notes/crc-programming-for-the-max14900e-octal-highspeed-industrial-switch.html)  
- [CRC Polynomial Zoo](https://users.ece.cmu.edu/~koopman/crc/crc32.html)  

#### Linear Feedback Shift Register References
- [Analog Devices: Pseudo RNG Using LFSR Note](https://www.analog.com/en/resources/design-notes/random-number-generation-using-lfsr.html)  
- [University of Otago: Table of LFSR Polynomials](https://www.physics.otago.ac.nz/reports/electronics/ETR2012-1.pdf)  

## Machine Learning Theory
### Math
- [Mathematics for Machine Learning eBook](https://mml-book.github.io/)  

## Software Engineering

### Massive Open Online Courses
- [MIT Missing Semester Course](https://missing.csail.mit.edu/)  
- [UCB: CS9F C++ for Programmers Course](https://selfpaced.bitbucket.io/#/cpp/calendar)  
- [UCB: CS9H Python for Programmers Course](https://selfpaced.bitbucket.io/#/python/calendar)  

### Git
- [Learn Git Branching Course](https://learngitbranching.js.org/)  
- [Pro Git eBook](https://git-scm.com/book/en/v2)  

### C++
- [A Computer Science Tapestry eBook](https://www2.cs.duke.edu/csed/tapestry/)  

### Python  
- [Think Python eBook](https://greenteapress.com/thinkpython2/html/)  

#### Python API References
- [NumPy and SciPy Documentation](https://docs.scipy.org/doc/)  
- [NumPy for MATLAB Guide](https://numpy.org/doc/stable/user/numpy-for-matlab-users.html)  
